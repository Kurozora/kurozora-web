...
class RelatedGames extends Component
{
    ...
    /**
     * The list of related games for the parent model.
     *
    ...
    public function getGameRelationsProperty(): Collection|LengthAwarePaginator
    {
        ...
        return $this->parent->gameRelations()
            ->with([
    ...
    {
        return match ($this->kind) {
            UserLibraryKind::Anime, UserLibraryKind::Manga => __('An extensive list of games of :x only on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => $this->parent->title, 'y' => config('app.name')]),
            UserLibraryKind::Game => __('An extensive list of sequel, prequel, mods, and dlc to :x only on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => $this->parent->title, 'y' => config('app.name')]),
        };
    }
    ...
    {
        return match ($this->kind) {
            UserLibraryKind::Anime, UserLibraryKind::Manga => __(':x’s Games', ['x' => $this->parent->title]),
            UserLibraryKind::Game => __(':x’s Related Games', ['x' => $this->parent->title]),
        };
    }
    ...
    /**
     * Returns the canonical URL for the active kind's related-games page.
     *
     * @return string
     */
    ...
    {
        return match ($this->kind) {
            UserLibraryKind::Anime => route('anime.related-games', $this->parent),
            UserLibraryKind::Manga => route('manga.related-games', $this->parent),
            UserLibraryKind::Game  => route('games.related-games', $this->parent),
        };
    }
    ...
    {
        return view('livewire.related-games');
    }
}
