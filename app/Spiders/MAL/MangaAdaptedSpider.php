...
use App\Processors\MAL\MangaAdaptedProcessor;
...
class MangaAdaptedSpider extends BasicSpider
{
    /**
    ...
    public array $startUrls = [
        'https://myanimelist.net/manga/adapted?type=all&page=1',
//        'https://myanimelist.net/manga/adapted?type=all&page=2',
    ];
    ...
    public function parse(Response $response): Generator
    {
        if ($response->getStatus() >= 400) {
            logger()->error('Manga Adapted;status:' . $response->getStatus());
            return $this->item([]);
        }

        $ids = $response->filter('.link-title')
