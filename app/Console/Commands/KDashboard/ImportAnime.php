...
class ImportAnime extends Command
{
    ...
    {
        $kAnime = KAnime::all();
        $oldCount = Anime::count('id');
        $this->info('Total old anime: ' . $oldCount);

        $this->withProgressBar($kAnime, function (KAnime $kAnime) {
            $anime = Anime::where([
                ['mal_id', $kAnime->id],
            ])->first();
        ...
        $newCount = Anime::count('id');

        $this->newLine();
        $this->info('Total new anime added: ' . $newCount - $oldCount);
        $this->info('Total anime: ' . $newCount);
