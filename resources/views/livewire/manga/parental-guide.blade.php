<main>
    ...
    <x-slot:meta>
        <meta property="og:title" content="{{ __('Parents Guide') }} | {{ $manga->title }} — {{ config('app.name') }}" />
        <meta property="og:description" content="{{ __(':x parental guide on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => $manga->title, 'y' => config('app.name')]) }}" />
        ...
        <link rel="canonical" href="{{ route('manga.parentalguide', $manga) }}">
    </x-slot:meta>
            ...
            <div>
                <div class="flex gap-1 pl-4 pr-4">
                    <div class="flex flex-wrap items-center w-full">
                        <h1 class="text-2xl font-bold">{{ __(':x’s Parents Guide', ['x' => $manga->title]) }}</h1>
                    </div>
                    ...
                    <ul class="m-0 space-y-4 list-none">
                        <li>
                            <div class="flex gap-1 items-center">
                                ...
                                <p class="text-secondary">{{ $manga->tv_rating->name }} ({{ $manga->tv_rating->description }})</p>
                            </div>
                        </li>
                        ...
                        @foreach (App\Enums\ParentalGuideCategory::getInstances() as $category)
                            <li>
                                <a href="#{{ $category->urlSlug() }}" class="flex gap-1 items-center">
                                    ...
                                    <p class="text-secondary">{{ $manga->parental_guide_stat->getAverageRating($category)->description }}</p>
                                </a>
                            </li>
                        @endforeach
                    </ul>
            ...
            <div class="flex flex-col gap-6">
                @foreach (App\Enums\ParentalGuideCategory::getInstances() as $category)
                    @php
                        ...
                        $averageRating = $manga->parental_guide_stat->getAverageRating($category);
                        [$averageRatingCount, $totalRatingCount] = $manga->parental_guide_stat->getAverageRatingCount($category);
                    @endphp
                    ...
                    <div id="{{ $categorySlug }}" class="flex flex-col gap-4 pb-6" style="scroll-margin-top: 4rem;">
                        <x-section-nav class="!mb-0">
                            ...
                            <x-slot:action>
                                <x-section-nav-link href="{{ route('manga.parentalguide.category', ['manga' => $manga, 'category' => $categorySlug]) }}">{{ __('See All') }}</x-section-nav-link>
                            </x-slot:action>
                        </x-section-nav>
