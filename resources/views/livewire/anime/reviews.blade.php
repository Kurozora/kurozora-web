<main>
    <x-slot:title>
        {!! __('Ratings & Reviews') !!} | {!! $anime->title !!}
    </x-slot:title>

    <x-slot:description>
        {{ __('Discover all :x reviews & ratings only on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => $anime->title, 'y' => config('app.name')]) }}
    </x-slot:description>

    <x-slot:meta>
        <meta property="og:title" content="{{ __('Ratings & Reviews') }} | {{ $anime->title }} — {{ config('app.name') }}" />
        <meta property="og:description" content="{{ __('Discover all :x reviews & ratings on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => $anime->title, 'y' => config('app.name')]) }}" />
        ...
        <link rel="canonical" href="{{ route('anime.reviews', $anime) }}">
    </x-slot:meta>

    <x-slot:appArgument>
        anime/{{ $anime->id }}/reviews
    </x-slot:appArgument>
            ...
            <div>
                <div class="flex gap-1 pl-4 pr-4">
                    <div class="flex flex-wrap items-center w-full">
                        <h1 class="text-2xl font-bold">{{ __(':x’s Ratings & Reviews', ['x' => $anime->title]) }}</h1>
                    </div>
        ...
        <section id="writeAReview" class="pb-8">
            ...
            <div class="flex flex-row flex-wrap gap-4 pl-4 pr-4">
                <div class="flex justify-between items-center">
                    ...
                    <livewire:components.star-rating :model-id="$anime->id" :model-type="$anime->getMorphClass()" :rating="$this->userRating?->rating" :star-size="'md'" />
                </div>
    ...
    <livewire:components.review-box :review-box-id="$reviewBoxID" :model-id="$anime->id" :model-type="$anime->getMorphClass()" :user-rating="$this->userRating" />
</main>
