                    ...
                    <ul class="m-0 space-y-4 list-none">
                        <li>
                            <div class="flex gap-1 items-center">
                                ...
                                <p class="text-secondary">{{ $anime->tv_rating->name }} ({{ $anime->tv_rating->description }})</p>
                            </div>
                        </li>
                        ...
                        @foreach (App\Enums\ParentalGuideCategory::getInstances() as $category)
                            <li>
                                <a href="#{{ $category->urlSlug() }}" class="flex gap-1 items-center">
                                    ...
                                    <p class="text-secondary">{{ $anime->parental_guide_stat->getAverageRating($category)->description }}</p>
                                </a>
                            </li>
                        @endforeach
                    </ul>
            ...
            <div class="flex flex-col gap-6">
                @foreach (App\Enums\ParentalGuideCategory::getInstances() as $category)
                    @php
                        ...
                        $averageRating = $anime->parental_guide_stat->getAverageRating($category);
                        [$averageRatingCount, $totalRatingCount] = $anime->parental_guide_stat->getAverageRatingCount($category);
                    @endphp
                    ...
                    <div id="{{ $categorySlug }}" class="flex flex-col gap-4 pb-6" style="scroll-margin-top: 4rem;">
                        <x-section-nav class="!mb-0">
                            ...
                            <x-slot:action>
                                <x-section-nav-link href="{{ route('anime.parentalguide.category', ['anime' => $anime, 'category' => $categorySlug]) }}">{{ __('See All') }}</x-section-nav-link>
                            </x-slot:action>
                        </x-section-nav>
