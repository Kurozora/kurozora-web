<main>
    <x-slot:title>
        {!! __('Ratings & Reviews') !!} | {!! $this->parent->title !!}
    </x-slot:title>

    <x-slot:description>
        {{ __('Discover all :x reviews & ratings only on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => $this->parent->title, 'y' => config('app.name')]) }}
    </x-slot:description>

    <x-slot:meta>
        <meta property="og:title" content="{{ __('Ratings & Reviews') }} | {{ $this->parent->title }} — {{ config('app.name') }}" />
        <meta property="og:description" content="{{ __('Discover all :x reviews & ratings on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => $this->parent->title, 'y' => config('app.name')]) }}" />
    ...
    <x-slot:appArgument>
        {{ $this->appArgumentSegment }}/{{ $this->parent->id }}/reviews
    </x-slot:appArgument>
    ...
    <div class="pb-6" wire:init="loadPage">
        <x-back-link
            ...
            :title="__(':x’s Ratings & Reviews', ['x' => $this->parent->title])"
        />
        ...
        <section id="ratingsAndReviews" class="pb-8 xl:safe-area-inset">
            <div class="flex flex-row flex-wrap justify-between gap-4 pl-4 pr-4">
                ...
                <div class="flex flex-col justify-end items-center text-center">
                    ...
                    <p class="font-bold text-2xl">{{ number_format($this->mediaStat->positivePercentage) }}%</p>
                    <p class="text-sm text-secondary">{{ $this->mediaStat->sentiment }}</p>
                </div>

                <div class="flex flex-col justify-end items-center text-center">
                    @svg('heart_fill', 'fill-current', ['width' => 32])
                    <p class="font-bold text-2xl">{{ number_format($this->mediaStat->favorite_share * 100) }}%</p>
                    <p class="text-sm text-secondary">{{ $this->mediaStat->favorite_sentiment }}</p>
                </div>
        ...
        <section id="writeAReview" class="pb-8">
            ...
            <div class="flex flex-row flex-wrap gap-4 pl-4 pr-4 xl:safe-area-inset-scroll">
                <div class="flex justify-between items-center">
                    ...
                    <livewire:components.rating-input :model-id="$this->parent->id" :model-type="$this->parent->getMorphClass()" :rating="$this->userRating?->rating" :star-size="'md'" :review-box-id="$reviewBoxID" />
                </div>
    ...
    <livewire:components.review-box :review-box-id="$reviewBoxID" :model-id="$this->parent->id" :model-type="$this->parent->getMorphClass()" :user-rating="$this->userRating" />
