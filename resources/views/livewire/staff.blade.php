<main>
    <x-slot:title>
        {{ __('Staff') }} | {!! $this->parent->title !!}
    </x-slot:title>

    <x-slot:description>
        {{ __('Discover all staff of :x only on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => $this->parent->title, 'y' => config('app.name')]) }}
    </x-slot:description>

    <x-slot:meta>
        <meta property="og:title" content="{{ __('Staff') }} | {{ $this->parent->title }} — {{ config('app.name') }}" />
        <meta property="og:description" content="{{ __('Discover all staff of :x on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => $this->parent->title, 'y' => config('app.name')]) }}" />
    ...
    <x-slot:appArgument>
        {{ $this->appArgumentSegment }}/{{ $this->parent->id }}/staff
    </x-slot:appArgument>
            ...
            <div>
                <div class="flex gap-1 pl-4 pr-4">
                    <div class="flex flex-wrap items-center w-full">
                        <h1 class="text-2xl font-bold">{{ __(':x’s Staff', ['x' => $this->parent->title]) }}</h1>
                    </div>
