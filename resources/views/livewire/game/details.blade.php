        ...
        <div class="relative">
            ...
            <div class="md:absolute md:bottom-0 md:left-0 md:right-0 lg:px-4">
                <div class="flex flex-nowrap pt-5 pb-8 pl-4 pr-4 md:mx-auto md:mb-8 md:p-6 md:max-w-lg md:bg-white md:bg-opacity-50 md:backdrop-filter md:backdrop-blur md:rounded-lg">
                    <x-picture :border="true" class="w-32 h-32 mr-2 rounded-3xl overflow-hidden" border-roundness="rounded-3xl">
                ...
                <div id="ratingBadge" class="flex-grow pr-12">
                    <a href="#ratingsAndReviews">
                        <p class="font-bold text-orange-500">
                            {{ number_format($game->mediaStat?->rating_average ?? 0, 1) }}
                        </p>
            ...
            <section id="ratingsAndReviews" class="pt-5 pb-8 pl-4 pr-4 border-t-2">
                ...
                <div class="flex flex-row justify-between">
                    <div class="text-center">
                        <p class="font-bold text-6xl">{{ number_format($game->mediaStat?->rating_average ?? 0, 1) }}</p>
                        <p class="font-bold text-sm text-gray-500">{{ __('out of') }} 5</p>
                    </div>

                    @auth
                        <div class="text-right">
                            <livewire:game.star-rating :game="$game" :rating="$game->ratings()->firstWhere('user_id', auth()->user()->id)?->rating" :star-size="'lg'" />
                            <p class="text-sm text-gray-500">{{ trans_choice('[0,1] Not enough ratings|[2,*] :x reviews', $game->mediaStat?->rating_count, ['x' => $game->mediaStat?->rating_count]) }}</p>
                        </div>
                    @endif
                </div>
            </section>
