<main>
    <x-slot:title>
        {{ $this->category->description }} | {{ __('Parents Guide') }} | {!! $game->title !!}
    </x-slot:title>
    ...
    <x-slot:meta>
        <link rel="canonical" href="{{ route('games.parentalguide.category', ['game' => $game, 'category' => $this->category->urlSlug()]) }}">
    </x-slot:meta>

    <x-slot:appArgument>
        games/{{ $game->id }}/parentalguide
    </x-slot:appArgument>
    ...
    <div class="pb-6">
        <section class="sticky top-0 mb-4 pt-4 pb-4 backdrop-blur bg-blur z-10 xl:safe-area-inset">
            <div class="flex flex-col gap-1 pl-4 pr-4">
                <a href="{{ route('games.parentalguide', $game) }}" class="text-secondary text-sm">
                    ← {{ __(':x’s Parents Guide', ['x' => $game->title]) }}
                </a>
