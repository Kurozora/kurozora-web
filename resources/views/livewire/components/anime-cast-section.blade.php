<div>
    @if ($castCount)
        <section class="pt-5 pb-8 px-4 border-t-2" wire:init="loadAnimeCast">
            <x-section-nav>
                <x-slot:title>
                    {{ __('Cast') }}
                </x-slot>
                ...
                <x-slot:action>
                    <x-simple-link href="{{ route('anime.cast', $anime) }}">{{ __('See All') }}</x-simple-link>
                </x-slot>
            </x-section-nav>
            ...
            <div class="grid grid-flow-col-dense gap-4 justify-start overflow-x-scroll no-scrollbar">
                @foreach($cast as $animeCast)
                    <x-lockups.cast-lockup :cast="$animeCast" />
                @endforeach
            </div>
        </section>
    @endif
