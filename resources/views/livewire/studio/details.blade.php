            ...
            <div class="relative xl:safe-area-inset">
                <div class="flex flex-col flex-wrap text-center items-center">
                    <picture
                        ...
                        style="height: 176px; background-color: {{ $studio->getFirstMedia(\App\Enums\MediaCollection::Profile)?->custom_properties['background_color'] ?? 'var(--bg-secondary-color)' }};"
                    >
        ...
        <section id="ratingsAndReviews" class="pb-8 xl:safe-area-inset">
            ...
            <div class="flex flex-row flex-wrap justify-between gap-4 pl-4 pr-4">
                ...
                <div class="flex flex-col justify-end items-center text-center">
                    ...
                    <p class="font-bold text-2xl">{{ number_format($studio->mediaStat->positivePercentage) }}%</p>
                    <p class="text-sm text-secondary">{{ $studio->mediaStat->sentiment }}</p>
                </div>

                <div class="flex flex-col justify-end items-center text-center">
                    @svg('heart_fill', 'fill-current', ['width' => 32])
                    <p class="font-bold text-2xl">{{ number_format($studio->mediaStat->favorite_share * 100) }}%</p>
                    <p class="text-sm text-secondary">{{ $studio->mediaStat->favorite_sentiment }}</p>
                </div>
        ...
        <section id="writeAReview" class="pb-8">
            ...
            <div class="flex flex-row flex-wrap gap-4 pl-4 pr-4 xl:safe-area-inset-scroll">
                <div class="flex justify-between items-center">
                    ...
                    <livewire:components.rating-input :model-id="$studio->id" :model-type="$studio->getMorphClass()" :rating="$userRating->first()?->rating" :star-size="'md'" :review-box-id="$reviewBoxID" />
                </div>
            ...
            <div class="mt-5">
                <livewire:sections.reviews :model="$studio" :review-box-id="$this->reviewBoxID" />
            </div>
        </section>
