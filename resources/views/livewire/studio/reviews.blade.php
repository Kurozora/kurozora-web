<main>
    ...
    <x-slot:description>
        {{ __('Discover all :x reviews & ratings only on Kurozora, the largest, free online anime, manga, game & music database in the world.', ['x' => $studio->name]) }}
    </x-slot:description>

    <x-slot:meta>
        <meta property="og:title" content="{{ __('Ratings & Reviews') }} | {{ $studio->name }} — {{ config('app.name') }}" />
        <meta property="og:description" content="{{ __('Discover all :x reviews & ratings on Kurozora, the largest, free online anime, manga, game & music database in the world.', ['x' => $studio->name]) }}" />
            ...
            <div>
                <div class="flex gap-1">
                    <div class="flex flex-wrap items-center w-full">
                        <h1 class="text-2xl font-bold">{{ __(':x’s Ratings & Reviews', ['x' => $studio->name]) }}</h1>
                    </div>
        ...
        <section id="writeAReview" class="mb-5 pt-5 border-t-2">
            <div class="flex flex-row flex-wrap gap-4">
                <div class="flex justify-between items-center">
                    ...
                    <livewire:components.star-rating :model-id="$studio->id" :model-type="$studio->getMorphClass()"  :rating="$this->userRating?->rating" :star-size="'md'" />
                </div>
    ...
    <livewire:components.review-box :review-box-id="$reviewBoxID" :model="$studio" :user-rating="collect([$this->userRating])" />
</main>
