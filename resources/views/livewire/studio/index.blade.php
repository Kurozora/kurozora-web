<main>
    ...
    <x-slot:description>
        {{ __('Discover an extensive list of anime studios, producers, and networks only on Kurozora, the largest, free online anime, manga, game & music database in the world.') }}
    </x-slot:description>

    <x-slot:meta>
        <meta property="og:title" content="{{ __('Studios') }} — {{ config('app.name') }}" />
        <meta property="og:description" content="{{ __('An extensive list of anime studios, producers, and networks only on Kurozora, the largest, free online anime, manga, game & music database in the world.') }}" />
    ...
    <div class="py-6" wire:init="loadPage">
        ...
        @else
            <section class="flex flex-col items-center mt-4 text-center">
                ...
                <p class="font-bold">{{ __('Studios Not Found') }}</p>

                <p class="text-sm text-secondary">{{ __('No studios found with the selected criteria.') }}</p>
            </section>
        @endif
    </div>
</main>
