<main>
    <x-slot:title>
        {{ $this->seasonOfYear->key . ' ' . $year }} | {{ $this->ogTitleNoun }}
    </x-slot:title>
    ...
    <x-slot:meta>
        <meta property="og:title" content="{{ $this->seasonOfYear->key . ' ' . $year }} | {{ $this->ogTitleNoun }} — {{ config('app.name') }}" />
    ...
    >
        ...
        <section id="mediaTypeHeader" class="bg-primary pt-4 pb-4 z-10">
            <x-season-pagination :type="$this->modelClass()" :season-of-year="$this->seasonOfYear" :year="$year" />
