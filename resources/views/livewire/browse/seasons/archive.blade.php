<main>
    <x-slot:title>
        {{ __('Seasonal Archive') }} | {{ $this->ogTitleNoun }}
    </x-slot:title>
    ...
    <x-slot:meta>
        <meta property="og:title" content="{{ __('Seasonal Archive') }} | {{ $this->ogTitleNoun }} — {{ config('app.name') }}" />
    ...
    <div class="pt-4 pb-6 xl:safe-area-inset">
        <h1 class="pl-4 pr-4 text-2xl font-bold">{{ $this->heading }}</h1>

        <section>
            <div id="mediaTypeHeader" class="pt-4 pb-5 bg-primary z-10">
                <x-season-pagination :type="$this->modelClass()" />
            </div>
                            ...
                            <tr class="{{ $year === now()->year ? 'bg-tinted' : '' }}">
                                ...
                                @foreach (\App\Enums\SeasonOfYear::asSelectArray() as $seasonOfYear)
                                    <td class="pt-2 pr-2 pb-2 pl-2 sm:p-4 {{ $year === now()->year && $seasonOfYear === season_of_year()->key ? 'font-semibold' : '' }}">
                                        <x-simple-link href="{{ $this->urlForYearSeason($year, $seasonOfYear) }}" wire:navigate>{{ $seasonOfYear }}</x-simple-link>
                                    </td>
                                @endforeach
                            </tr>
                        @endforeach
