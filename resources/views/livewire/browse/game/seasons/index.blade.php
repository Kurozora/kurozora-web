<main>
    <x-slot:title>
        {{ $this->seasonOfYear->key . ' ' . $year }} | {{ __('Games') }}
    </x-slot:title>

    <x-slot:description>
        {{ __('Browse the :x :y game season. Join the :z community and create your anime, manga and game list. Discover songs, episodes and read reviews and news!', ['x' => $this->seasonOfYear->key, 'y' => $year, 'z' => config('app.name')]) }}
    </x-slot:description>

    <x-slot:meta>
        <meta property="og:title" content="{{ $this->seasonOfYear->key . ' ' . $year }} | {{ __('Games') }} — {{ config('app.name') }}" />
        ...
        <link rel="canonical" href="{{ route('games.seasons.year.season', [$year, $season]) }}">
    </x-slot:meta>
    ...
    >
        <section class="flex gap-1 pl-4 pr-4">
            <div class="flex flex-wrap items-center w-full">
                <h1 class="text-2xl font-bold">{{ __('Seasonal Games') }}</h1>
            </div>
            ...
            <section class="space-y-10">
                @foreach ($this->mediaTypes as $mediaType)
                    <div x-show="selectedMediaType === '{{ $mediaType->name }}' || selectedMediaType === null">
                        <livewire:components.browse.seasons-section :class="\App\Models\Game::class" :media-type="$mediaType" :season-of-year="$this->seasonOfYear->value" :year="$year" />
                    </div>
                @endforeach
            </section>
        @else
            <section class="flex flex-col items-center justify-center mt-4 text-center" style="min-height: 50vh;">
                <x-picture>
                    <img class="w-full max-w-sm" src="{{ asset('images/static/placeholders/empty_game_library.webp') }}" alt="Empty Game Season" title="Empty Game Season">
                </x-picture>

                <p class="font-bold">{{ __('No Games') }}</p>

                <p class="text-sm text-secondary">{{ __('There are no games publishing this season.') }}</p>
            </section>
        @endif
    </div>
