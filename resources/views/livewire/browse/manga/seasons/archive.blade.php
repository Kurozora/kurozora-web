<main>
    ...
    <x-slot:description>
        {{ __('Browse the archive of manga seasons. Join the :x community and create your anime, manga and game list. Discover songs, episodes and read reviews and news!', ['x' => config('app.name')]) }}
    </x-slot:description>

    <x-slot:meta>
        <meta property="og:title" content="{{ __('Seasonal Archive') }} | {{ __('Manga') }} — {{ config('app.name') }}" />
        <meta property="og:description" content="{{ __('Browse the archive of manga seasons. Join the :x community and create your anime, manga and game list. Discover songs, episodes and read reviews and news!', ['x' => config('app.name')]) }}" />
    ...
    <div class="py-6">
        <h1 class="pl-4 pr-4 text-2xl font-bold">{{ __('Seasonal Manga Archive') }}</h1>
