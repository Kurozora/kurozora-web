    ...
    >
        ...
        @else
            <section class="flex flex-col items-center justify-center mt-4 text-center" style="min-height: 50vh;">
                <x-picture>
                    <img class="w-full max-w-sm" src="{{ asset('images/static/placeholders/empty_anime_library.webp') }}" alt="Empty Anime Season" title="Empty Anime Season">
                </x-picture>

                <p class="font-bold">{{ __('No Anime') }}</p>

                <p class="text-sm text-secondary">{{ __('There are no anime airing this season.') }}</p>
            </section>
        @endif
    </div>
