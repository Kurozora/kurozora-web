<main>
    ...
    <x-slot:description>
        {{ __('Discover the extensive list of games that :x appears in only on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => config('app.name'), 'x', $character->name, 'y' => config('app.name')]) }}
    </x-slot:description>

    <x-slot:meta>
        <meta property="og:title" content="{{ __('Games') }} | {{ $character->name }} — {{ config('app.name') }}" />
        <meta property="og:description" content="{{ __('Discover the extensive list of games that :x appears in only on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => config('app.name'), 'x', $character->name, 'y' => config('app.name')]) }}" />
