<main>
    <x-slot:title>
        {!! __('Ratings & Reviews') !!} | {!! $character->name !!}
    </x-slot:title>

    <x-slot:description>
        {{ __('Discover all :x reviews & ratings only on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => $character->name, 'y' => config('app.name')]) }}
    </x-slot:description>

    <x-slot:meta>
        <meta property="og:title" content="{{ __('Ratings & Reviews') }} | {{ $character->name }} — {{ config('app.name') }}" />
        <meta property="og:description" content="{{ __('Discover all :x reviews & ratings on :y, the largest, free online anime, manga, game & music database in the world.', ['x' => $character->name, 'y' => config('app.name')]) }}" />
        ...
        <link rel="canonical" href="{{ route('characters.reviews', $character) }}">
    </x-slot:meta>
    ...
    <div class="pb-6" wire:init="loadPage">
        <x-back-link
            ...
            :title="__(':x’s Ratings & Reviews', ['x' => $character->name])"
        />
        ...
        <section id="writeAReview" class="pb-8">
            ...
            <div class="flex flex-row flex-wrap gap-4 pl-4 pr-4 xl:safe-area-inset-scroll">
                <div class="flex justify-between items-center">
                    ...
                    <livewire:components.rating-input :model-id="$character->id" :model-type="$character->getMorphClass()"  :rating="$this->userRating?->rating" :star-size="'md'" :review-box-id="$reviewBoxID" />
                </div>
    ...
    <livewire:components.review-box :review-box-id="$reviewBoxID" :model-id="$character->id" :model-type="$character->getMorphClass()" :user-rating="$this->userRating" />
</main>
