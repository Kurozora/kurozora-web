<main>
    ...
    @php
        $profileMedia = $character->getFirstMedia(\App\Enums\MediaCollection::Profile);
    @endphp

    <div class="pb-6" wire:init="loadPage">
        <section
            class="flex items-center justify-center relative pt-10 pb-10 pl-4 pr-4"
            style="height: 24rem; background-color: {{ $profileMedia?->custom_properties['background_color'] ?? 'var(--bg-secondary-color)' }}"
        >
            ...
            <div class="relative xl:safe-area-inset">
                <div class="flex flex-col flex-wrap text-center items-center">
                    <picture
                        ...
                        style="height: 176px; background-color: {{ $profileMedia?->custom_properties['background_color'] ?? 'var(--bg-secondary-color)' }};"
                    >
                        <img class="w-full h-full object-cover lazyload" style="object-position: {{ $profileMedia?->objectPositionStyle() ?? 'center' }};" data-sizes="auto" data-src="{{ $character->getFirstMediaFullUrl(\App\Enums\MediaCollection::Profile()) ?? asset('images/static/placeholders/person_poster.webp') }}" alt="{{ $character->name }} Profile Image" title="{{ $character->name }}">
        ...
        <section id="ratingsAndReviews" class="pb-8 xl:safe-area-inset">
            ...
            <div class="flex flex-row flex-wrap justify-between gap-4 pl-4 pr-4">
                ...
                <div class="flex flex-col justify-end items-center text-center">
                    ...
                    <p class="font-bold text-2xl">{{ number_format($character->mediaStat->positivePercentage) }}%</p>
                    <p class="text-sm text-secondary">{{ $character->mediaStat->sentiment }}</p>
                </div>

                <div class="flex flex-col justify-end items-center text-center">
                    @svg('heart_fill', 'fill-current', ['width' => 32])
                    <p class="font-bold text-2xl">{{ number_format($character->mediaStat->favorite_share * 100) }}%</p>
                    <p class="text-sm text-secondary">{{ $character->mediaStat->favorite_sentiment }}</p>
                </div>
        ...
        <section id="writeAReview" class="pb-8">
            ...
            <div class="flex flex-row flex-wrap gap-4 pl-4 pr-4 xl:safe-area-inset-scroll">
                <div class="flex justify-between items-center">
                    ...
                    <livewire:components.rating-input :model-id="$character->id" :model-type="$character->getMorphClass()" :rating="$userRating->first()?->rating" :star-size="'md'" :review-box-id="$reviewBoxID" />
                </div>
            ...
            <div class="mt-5">
                <livewire:sections.reviews :model="$character" :review-box-id="$this->reviewBoxID" />
            </div>
        </section>
