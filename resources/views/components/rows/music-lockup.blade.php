...
<div {{ $attributes->merge(['class' => 'flex gap-4 justify-between ' . $class]) }}>
    ...
    @foreach($mediaSongs as $index => $mediaSong)
        <x-lockups.music-lockup :song="$mediaSong->song" :anime="$mediaSong->anime" :type="$mediaSong->type" :position="$mediaSong->position" :episodes="$mediaSong->episodes" :show-episodes="$showEpisodes" :show-model="$showModel" :rank="($page - 1) * $perPage + $index + 1" :is-ranked="$isRanked" :is-row="$isRow" />
    @endforeach
