...
<div {{ $attributes->merge(['class' => 'relative w-full pb-2']) }}>
    <div class="flex flex-col">
        <picture class="relative w-28 h-40 rounded-lg shadow-md overflow-hidden md:w-32 md:h-48">
            <img class="w-full h-full object-cover lazyload" data-sizes="auto" data-src="{{ $character->poster_image_url ?? asset('images/static/placeholders/person_poster.webp') }}" alt="{{ $character->name }} Profile Picture" title="{{ $character->name }}">
