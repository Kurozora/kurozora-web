    ...
    <div class="flex flex-nowrap">
        <section class="relative flex">
            <picture class="relative shrink-0 w-28 h-40 mr-2 rounded-lg overflow-hidden md:w-32 md:h-48">
                <img class="w-full h-full object-cover lazyload" data-sizes="auto" data-src="{{ $cast->person->getFirstMediaFullUrl(\App\Enums\MediaCollection::Profile()) ?? asset('images/static/placeholders/person_poster.webp') }}" alt="{{ $cast->person->full_name }} Profile Image" title="{{ $cast->person->full_name }}">
